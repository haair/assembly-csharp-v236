...
public class Panel : IActionListener, IChatable
{
    ...
    public void addThachDauDetail(TopInfo t)
    {
        ...
        text = text + "\n|1|" + t.info;
        text = text + "\n|2|" + t.info2;
        if (text.IndexOf(mResources.HP) >= 0 && text.IndexOf(mResources.HP + ":") < 0)
        {
            text = Res.replace(text, mResources.HP, mResources.HP + ":");
        }
        if (text.IndexOf(mResources.hit_point) >= 0 && text.IndexOf(mResources.hit_point + ":") < 0)
        {
            text = Res.replace(text, mResources.hit_point, mResources.hit_point + ":");
        }
        if (text.IndexOf(mResources.armor) >= 0 && text.IndexOf(mResources.armor + ":") < 0)
        {
            text = Res.replace(text, mResources.armor, mResources.armor + ":");
        }
        int num = t.info2.IndexOf(":") + 1;
        int length = t.info2.Length - num;
        string text2 = t.info2.Substring(num, length);
    ...
    {
        mFont.tahoma_7b_white.drawString(g, "HP: " + NinjaUtil.getMoneys(Char.myPetz().cHP) + "/" + NinjaUtil.getMoneys(Char.myPetz().cHPFull), X + 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
        mFont.tahoma_7b_white.drawString(g, "MP: " + NinjaUtil.getMoneys(Char.myPetz().cMP) + "/" + NinjaUtil.getMoneys(Char.myPetz().cMPFull), X + 60, 16, mFont.LEFT, mFont.tahoma_7b_dark);
        mFont.tahoma_7_yellow.drawString(g, mResources.critical + ": " + Char.myPetz().cCriticalFull + "   " + mResources.armor + ": " + NinjaUtil.getMoneys(Char.myPetz().cDefull), X + 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
    ...
    public void perform(int idAction, object p)
    {
        ...
        if (idAction == 8004)
        {
            ...
            TeleportHelper.TeleportToPlayer(infoItem3.charInfo.charID);
        }
        if (idAction == 8001)
        {
